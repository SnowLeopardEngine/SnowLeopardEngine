...
namespace SnowLeopardEngine::Editor
{
    ...
    DirectoryInformation::DirectoryInformation(const std::string& thisPath, DirectoryInformation* parent) : Parent(parent)
    {
        ThisPath = thisPath;
        if (Parent) {
        ...
        if(ImGui::Begin(ICON_MDI_FOLDER_STAR "Resources"))
        {
            ...
            if(ImGui::BeginDragDropTarget()) {
                ...
                if(data) {
                    ...
                    if(MoveFFile(file, m_MovePath)) {
                        SNOW_LEOPARD_INFO("Moved File: "+ file + " to " + m_MovePath);
                        //std::cout<<"Moved File: "<<file<<" to "<<m_MovePath;
                    }
                    m_IsDragging = false;
                }
            ...
            ImGui::BeginChild("##directory_structure", ImVec2(0, ImGui::GetWindowHeight() - ImGui::GetFrameHeightWithSpacing() * 2.6f - offset));
            {
                ...
                ImGui::EndChild();
                {
                    ...
                    m_GridItemsPerRow = static_cast<int>(floor(xAvail / (m_GridSize + ImGui::GetStyle().ItemSpacing.x)));
                    ...
                    if(m_IsInListView) {
                        for(int i = 0; i < m_CurrentDir->Children.size(); i++) {
                            if(!m_CurrentDir->Children.empty()) {
                                ...
                                bool doubleClicked = RenderFile(i, m_CurrentDir->Children[i]->IsDirectory, shownIndex, !m_IsInListView);
                                if(doubleClicked)
                                    break;
                                shownIndex++;
                            }
            ...
            if(ImGui::BeginDragDropTarget()) {
                ...
                if(data) {
                    ...
                    if(MoveFFile(a, m_MovePath)) {
                        SNOW_LEOPARD_INFO("Moved File: " + a + " to " + m_MovePath);
                        //std::cout<<"Moved File: "<<a<<" to "<<m_MovePath;
                    }
                    m_IsDragging = false;
                }
                ImGui::EndDragDropTarget();
    ...
    bool ResourcePanel::RenderFile(int dirIndex, bool folder, int shownIndex, bool gridView)
    {
        ...
        if(gridView) {
            ...
            ImGui::SameLine();
            if(ImGui::Selectable(m_CurrentDir->Children[dirIndex]->ThisPath.c_str(),false, ImGuiSelectableFlags_AllowDoubleClick)) {
                if(ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left)) {
                    doubleClicked = true;
                }
            }
    ...
    DirectoryInformation* ResourcePanel::ProcessDirectory(const std::string& directoryPath, DirectoryInformation* parent)
    {
            ...
            if(isOpen && containsFolder) {
                ...
                for(auto *i : dirInfo->Children) {
                    if(i->IsDirectory) {
                        ...
                        const ImRect childRect = ImRect(currentPos.x, currentPos.y, currentPos.x + sizeToAdd.x, currentPos.y + sizeToAdd.y);
