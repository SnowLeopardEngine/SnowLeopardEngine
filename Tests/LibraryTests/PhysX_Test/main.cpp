...
int main()
{
    ...
    // create foundation
    auto* const foundation = PxCreateFoundation(PX_PHYSICS_VERSION, allocator, errorCallback);
    if (!foundation)
    {
    ...
    auto* const pvd = PxCreatePvd(*foundation);
    ...
    // create physics
    auto* const physics = PxCreatePhysics(PX_PHYSICS_VERSION, *foundation, PxTolerancesScale(), true, pvd);
    if (!physics)
    {
